  ...
  return (
    <Box bg="gray.400">
      <Checkbox
        ...
        onChange={() => {
          const prev = { ...fixture };
          initializeState(prev);
          prev.state = !prev.children.every((child) =>
            child.children.every(truethy)
          );

          prev.children.forEach((child) => {
            initializeState(child);
            child.state = prev.state;
            child.children.forEach((gChild) => {
              initializeState(gChild);
              gChild.state = prev.state;
            });
          });
            ...
            <div key={child.id}>
              <Checkbox
                ...
                onChange={(e) => {
                  const prev = { ...fixture };
                  initializeState(child);
                  console.log(!prev.children[index].state);
                  prev.children[index].state = !prev.children[index].state;
                  ...
                  return (
                    <Checkbox
                      ...
                      onChange={(e) => {
                        ...
                        prev.children[index].state = prev.children[
                          index
                        ].children.some(truethy);
                        setFixture(prev);
                      }}
                    >
                      {grand.name}
